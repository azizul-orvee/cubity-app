    ...
    <div className="grid gap-5">
      ...
      <section className="rounded-3xl bg-white p-4 ring-1 ring-border">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link href={receivables.clientsList("due")} className="text-sm font-medium text-primary">
            All
          </Link>
        </div>
        <HorizontalBars
      ...
      <section className="grid gap-4">
        ...
        <QueueCard
          ...
          rows={snapshot.upcomingClients.map((client) => ({
            id: client.id,
            title: client.name,
            meta: client.status.promisedPartial
              ? `of ${formatMoney(client.status.outstanding)} · ${formatDate(client.status.promised)}`
              : formatDate(client.status.promised),
            amount: client.status.promisedAmount,
            href: receivables.client(client.id),
            status: client.status,
          }))}
        />
